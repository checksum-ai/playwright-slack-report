...
test.describe('ResultsParser', () => {
  ...
  test('handles bug and recovered statuses correctly', async ({}) => {
    ...
    expect(results.bug).toEqual(1);
    expect(results.recovered).toEqual(1);
    expect(results.tests).toHaveLength(2);
    expect(results.tests[0].status).toEqual('bug');
    expect(results.tests[1].status).toEqual('recovered');

    // Bug should appear in failures, recovered should not
    const failures = await resultsParser.getFailures();
    expect(failures).toHaveLength(1);
    expect(failures[0].test).toContain('Bug Test');
  });
});
