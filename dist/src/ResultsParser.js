...
class ResultsParser {
    ...
    async parseFromJsonFile(filePath) {
        ...
        if (parsedData.checksumMetadata?.tests) {
            ...
            flakyCount = parsedData.checksumMetadata.tests.filter((test) => test.outcome === 'flaky' && (!test.recovered || test.bug)).length;
            // Passed: tests with outcome="expected"
            passedCount = parsedData.checksumMetadata.tests.filter((test) => test.outcome === 'expected').length;
    ...
    async parseTests(suiteName, specs, retries, suite) {
        ...
        for (const spec of specs) {
            for (const test of spec.tests) {
                ...
                isRecovered = test.annotations?.some((annotation) => annotation.type && (annotation.type.includes('auto-recovered') ||
