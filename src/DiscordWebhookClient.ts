...
export default class DiscordWebhookClient {
  ...
  private async sendWebhookRequest(payload: DiscordWebhookPayload, retryCount = 0): Promise<Response> {
    try {
      ...
      if (!response.ok) {
        if (response.status === 429 && retryCount < DiscordWebhookClient.MAX_RETRIES) {
  ...
  private generatePayload(summaryResults: SummaryResults, maxNumberOfFailures: number): DiscordWebhookPayload {
    ...
    const payload: DiscordWebhookPayload = {
      ...
      embeds: [{
        ...
        color: this.webhookConfig.embedColor ? parseInt(this.webhookConfig.embedColor.replace('#', ''), 16) : statusColor,
        fields: [
          {
    ...
    if (summaryResults.meta && summaryResults.meta.length > 0) {
      payload.embeds[0].fields.push({
        ...
        value: summaryResults.meta.map(m => `**${m.key}**: ${m.value}`).join('\n'),
        inline: false
      });
    }
